...
**작성일**: 2026-07-11
**상태**: 초안 (v1)
**목적**: 포트폴리오급 실서비스 구현 및 배포. 핵심 증명 포인트는 **동일 자산·잔액에 대량 트래픽이 몰릴 때의 Race Condition을 세 가지 락 전략으로 각각 해결하고, 정량 비교한 것**.
...
3. **저비용 고가용 인프라**: EC2 스팟 인스턴스만으로 운영하되, 스팟 회수 이벤트·스냅샷 처리로 가용성을 유지한다. IaC로 전체를 코드화하여 AWS 크레딧 소진 시 GCP/Azure로 이전 가능하게 한다.
...
| IaC | Terraform(OpenTofu) + k3s + Helm | 멀티클라우드 이전 대비 |
...
- 도구: k6. 시나리오 S1~S4를 각 락 전략 × 동시성 단계(VU 10/50/200/500)로 실행.
...
| 항목 | 예상 |
|---|---|
| 스팟 t4g.medium x3 (us-east-2) | ~$12–18 |
| EBS gp3 30GB x2 + 스냅샷 | ~$6 |
| S3 + 트래픽 | ~$3 |
| Cloudflare | $0 |
| **합계** | **~$25 내외** (프리티어/크레딧 적용 전) |
...
| M5 | Terraform + k3s + Cloudflare Tunnel 배포, 스팟 회수 훈련 | 인스턴스 강제 종료 후 5분 내 자동 복구 |
| M6 | k6 벤치마크 + Grafana 대시보드 + README 비교 보고서 | 락 전략 비교표/그래프 공개 |
| M7 (선택) | GCP 이전 리허설 | 이전 소요 시간 문서화 |
